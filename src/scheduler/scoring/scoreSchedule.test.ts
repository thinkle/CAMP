...
import { CapacityError, DuplicateError, scoreSchedule, validateSchedule } from './scoreSchedule';
...
describe(
    ...
    () => {        
        ...
        test("Valid schedule should return null", () => {
            expect(
                validateSchedule(validSchedule, activities)
            ).toBe(null);
        });

        test("Overbooked schedule should return CapacityError", () => {
            expect(
                validateSchedule(overbookedSchedule, activities)
            ).toBe(CapacityError);
        });

        test("Schedule with duplicate student should return DuplicateError", () => {
            expect(
                validateSchedule(dupedSchedule, activities)
            ).toBe(DuplicateError);
        });
    }
)
...
describe(
    "Score schedules using summed preferences",
    () => {
        test("Activity scores sum correctly", () => {
            ...
            expect(                
                scoreSchedule(schedule, studentPreferences)
            ).toBe(144);

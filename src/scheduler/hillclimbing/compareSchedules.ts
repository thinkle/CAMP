...
export const compareSchedules = (schedule1 : Schedule, schedule2 : Schedule) : Similarity => {
...
export const getAssignmentSimilarity = (schedule1 : Schedule, schedule2 : Schedule) : number => {
    let sameCount = 0;
    for (let i=0; i<schedule1.length; i++) {
        if (schedule1[i].student !== schedule2[i].student) {
            throw new Error('Schedules must have the same students in the same order');            
        }
...
export const getCohortSimilarity = (schedule1: Schedule, schedule2: Schedule): number => {
    ...
    for (let cohort1 of cohortArray1) {
        ...
        for (let cohort2 of cohortArray2) {
            ...
            const intersectionSize = [...cohort1].filter(student => cohort2.has(student)).length;

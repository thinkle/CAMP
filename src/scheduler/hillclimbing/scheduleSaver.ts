...
export const scheduleToId = (schedule : Schedule, activities : Activity[]) : string => {
    ({ schedule, activities } = normalizeScheduleAndActivities(schedule, activities));
...
export const idToSchedule = (id : string, studentPreferences : StudentPreferences[], activities : Activity[]) : Schedule => {
    let schedule = studentPreferences.map(s => ({student: s.identifier,activity:""}));
    ({ schedule, activities } = normalizeScheduleAndActivities(schedule, activities));    
    const scheduleArray = JSON.parse(atob(id));
    if (scheduleArray.length !== schedule.length) {
        throw new Error(`Invalid schedule ID: ${id}, expected ${schedule.length} activities and got ${scheduleArray.length}`);
    }
...
function normalizeScheduleAndActivities(schedule: Schedule, activities: Activity[]) {

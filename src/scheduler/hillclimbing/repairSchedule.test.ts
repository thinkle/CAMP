...
describe("repairSchedule", () => {
  it("should remove students from overflowing activities", () => {
    ...
    const preferences: StudentPreferences[] = [
      {
        ...
        activity: [
          ...
          { activity: "activity2", weight: 1 },
        ],
        peer: [],
      },
      {
        identifier: "student2",
        activity: [{ activity: "activity1", weight: 1 }],
        peer: [],
      },
      {
        identifier: "student3",
    ...
    expect(repairedSchedule).toEqual(
      expect.arrayContaining([{ student: "student1", activity: "activity2" }])
    );
  });

  it("should assign unassigned students to their preferred activities", () => {
    const schedule: Schedule = [{ student: "student1", activity: "activity1" }];
    const preferences: StudentPreferences[] = [
    ...
    const activities: Activity[] = [{ activity: "activity1", capacity: 2 }];

    const repairedSchedule = repairSchedule(schedule, preferences, activities);

    expect(repairedSchedule.length).toBe(2);
    expect(repairedSchedule).toEqual(
      expect.arrayContaining([{ student: "student2", activity: "activity1" }])
    );
  });
  ...
  it("should throw an error if no available activities for a student", () => {
    ...
    const activities: Activity[] = [{ activity: "activity1", capacity: 0 }];

    expect(() =>
      repairSchedule(schedule, preferences, activities)
    ).toThrowError("No available activities for student: student1");
  });
...
it("should assign students to alternative activities if their preferred one is full", () => {
  ...
  expect(repairedSchedule.length).toBe(3);
  expect(repairedSchedule).toEqual(
    expect.arrayContaining([{ student: "student3", activity: "activity2" }])
  );
});

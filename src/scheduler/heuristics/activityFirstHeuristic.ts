...
import type { StudentPreferences, Activity, Assignment, Schedule } from "../../types";

export const assignByActivity = (prefs : StudentPreferences[], activities : Activity[]) : Schedule => {
    ...
    for (const pref of prefs) {        
        const preferredActivities = pref.activity.sort((a, b) => b.weight - a.weight);
        let assigned = false;
        ...
        if (!assigned) {
            throw new Error(`No available activities for student: ${pref.identifier}`);
        }
    }
    return assignments;
}

...
describe("Numeric Activity ID Type Mismatch Bug", () => {
  test("BUG REPRODUCTION: should match student activity preferences (FAILS before fix)", () => {
    ...
    for (const assignment of scheduleWithBug) {
      const student = studentPreferences.find(
        (s: any) => s.identifier === assignment.student
      );
      const hasPreference = student.activity.some(
        (a: any) => a.activity === assignment.activity
      );
      if (hasPreference) {
        matchCountWithBug++;
      }
  ...
  test("should succeed with string activity IDs (after fix)", () => {
    ...
    const activityFirstSchedule = assignByActivity(
      studentPreferences,
      activities
    );
    ...
    const mutualPeerSchedule = assignMutualPeersFirst(
      studentPreferences,
      activities
    );
  ...
  test("real-world scenario: 37 students with numeric cabin IDs", () => {
    ...
    const studentPreferences: StudentPreferences[] = studentNames.map(
      (name, i) => ({
        identifier: name,
        activity: [
          { activity: "201", weight: 0 },

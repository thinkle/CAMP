...
describe("assignByActivity", () => {
  ...
  it("should throw an error if a student has no available activities", () => {
    ...
    expect(() => assignByActivity(prefs, activities)).toThrow(
      "No available activities for student: student3"
    );
  });
  ...
  it("should throw an error if an unknown activity is encountered", () => {
    const prefs: StudentPreferences[] = [
      {
        identifier: "student1",
        peer: [],
        activity: [{ activity: "unknownActivity", weight: 10 }],
      },
    ];
    ...
    expect(() => assignByActivity(prefs, activities)).toThrow(
      "Unknown activity: unknownActivity"
    );
  });
});

<script lang="ts">
  import SimilarityIndicator from './SimilarityIndicator.svelte';
    ...
    import { Button, Bar, Select, TabBar, TabItem, Progress, FormItem, MiniButton, Table, Checkbox } from 'contain-css-svelte';
    ...
    function calculateDifferenceGrid (ss : ScheduleInfo[]) : Similarity[][] {
      ...
      for (let i = 0; i < ss.length; i++) {
        ...
        for (let j = 0; j <= i; j++) {
          if (i === j) {
            grid[i][j] = { assignmentSimilarity: 1, cohortSimilarity: 1 }; // identical schedules
          } else {

<script lang="ts">
  ...
  import { Bar, Button, FormItem, Input, MiniButton, Select, TabBar, TabItem, Table} from "contain-css-svelte";
  ...
  let activityRequestsByWeight : {[key : number] : StudentPreferences[] }= {}
...
<div>
    <TabBar>
        <TabItem active={mode == 'students'} on:click={() => mode = 'students'}>Students</TabItem>
        <TabItem active={mode == 'activity'} on:click={() => mode = 'activity'}>Activity</TabItem>
    </TabBar>
    {#if mode == 'students'}
        <Bar>
        ...
        <FormItem>
            ...
            <div slot="after" style="display:flex; align-items: center; gap: 4px">
                <MiniButton on:click={() => sp = data.studentPreferences[(data.studentPreferences.indexOf(sp) - 1) % data.studentPreferences.length]}>
                    &lt;
                </MiniButton>
                <MiniButton on:click={() => sp = data.studentPreferences[(data.studentPreferences.indexOf(sp) + 1) % data.studentPreferences.length]}>
                    &gt;
    ...
    {:else if mode == 'activity'}
        ...
        <FormItem>
            ...
            <div slot="after" style="display:flex; align-items: center; gap: 4px">
                <MiniButton on:click={() => activity = data.activities[(data.activities.indexOf(activity) - 1) % data.activities.length]}> 
                    &lt;
                </MiniButton>
                <MiniButton on:click={() => activity = data.activities[(data.activities.indexOf(activity) + 1) % data.activities.length]}> 
                    &gt;
